...
class StaticChecker(BaseVisitor, Utils):
    ...
    def visitVarDecl(self, ast, env):
        ...
        vartype = ast.varType
        if not vartype:
            vartype = self.expr_checker.check(ast.varInit, env)
        
        env[0].append(Symbol(ast.varName, VType(vartype)))
        return None
    ...
    def visitConstDecl(self, ast, env):
        ...
        contype = ast.conType
        if not contype:
            contype = self.expr_checker.check(ast.iniExpr, env)
        
        env[0].append(Symbol(ast.conName, CType(contype)))
        return None
    ...
    def visitFuncDecl(self, ast, env):
        ...
        # body: Block
        params = list(map(
            lambda param: Symbol(param.parName, VType(params.parType)),
            ast.params,
        ))
